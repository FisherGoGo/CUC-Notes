# Lec01 The Geometry of Linear Equations
...
左侧矩阵 $A$ 称为系数矩阵，$\mathbf{x}$ 称为未知数向量，方程右边的数组成了向量 $\mathbf{b}$：
...
这个问题等价于：$A$ 的列向量的线性组合是否能够填充满所有的 $xy$ 平面

对于这里的二阶方阵，当上述问题答案为 **不** 时，我们称矩阵 $A$ 为奇异矩阵，此时列向量为线性相关，这些列向量的所有线性组合，在二维情况下只会落在某一点或者某一直线上。即不能生成整个空间