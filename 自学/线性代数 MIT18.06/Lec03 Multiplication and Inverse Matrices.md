...
这个方法在前两节课经常提到过，一个矩阵左乘一个列向量，可以看出矩阵若看列的线性组合成一个新的列向量，一个矩阵右乘一个行向量也是如此
...
可以发现对于答案矩阵，每一列都是其他列的倍数，每一行也是其他行的倍数，或者是每一行都是左侧行向量的倍数，每一列都是左侧列向量的倍数，那么对于普通的矩阵乘法就可以写成：

$$AB=\sum\limits_{k=1}^{n} \begin{bmatrix}a_{1k}\\a_{2k}\\\vdots\\a_{mk}\end{bmatrix} \begin{bmatrix}b_{k1}&b_{k2}&\ldots&b_{kn}\end{bmatrix}$$
...
$$\begin{bmatrix}1&3\\2&7\end{bmatrix}_{A} \begin{bmatrix}a&b\\c&d\end{bmatrix}_{A^{-1}}=\begin{bmatrix}1&0\\0&1\end{bmatrix}_{I} $$

我们的目的是解出 $A$ ，可以看出解 $n$ 个线性方程，即 $A$ 乘以 $A^{-1}$ 的第 $j$ 列得到 $I$ 的第 $j$ 列，这与 Lec02 里讲的相似，但是我们想要一下子解决 $n$ 个线性方程，于是有了如下方法

## Gauss-Jordan Elimination

我们做如下矩阵：

$$\begin{bmatrix}A&I\end{bmatrix}=\begin{bmatrix}1&3&1&0\\2&7&0&1\end{bmatrix}$$

然后对这个矩阵进行变换，使得其变成 $\begin{bmatrix}I&E\end{bmatrix}$ ，那么 $A^{-1}=E$ ，为什么这样子可行

因为做变换相当于左乘变换矩阵，即 $E \begin{bmatrix}A&I\end{bmatrix}=\begin{bmatrix}I&E\end{bmatrix}$ ，那么 $EA=I$ ，根据定义 $E$ 为 $A$ 的逆矩阵
