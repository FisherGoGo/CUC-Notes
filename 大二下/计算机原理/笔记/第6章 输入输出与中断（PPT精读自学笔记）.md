...
> 易错：数据信息、状态信息、控制信息虽然名字不同，但 CPU 与接口交换时通常都通过数据总线传送；地址总线负责选端口，控制总线负责指出读/写和 I/O 操作类型。
...
### 2.3 IN/OUT 指令的总线信号 [高频]

8086/8088 访问 I/O 端口时，题目常问控制信号电平。记住两点：
...
| 信号 | 电平 | 原因 |
|---|---|---|
| `M/IO` | 低 | 访问 I/O，不是访问存储器 |
| `/RD` | 高 | 不是读操作，读信号无效 |
| `/WR` | 低 | I/O 写操作，写信号有效 |

> 易错：`OUT` 是 CPU 向端口写数据，所以有效的是 `/WR`；`IN` 才是 `/RD` 有效。
...
从“程序控制方式”这个口径看，常把数据传送分成：

```text
程序控制方式：无条件传送、查询传送、中断传送
DMA 方式：硬件控制为主，不归入程序控制方式
```

> 易错：填空题若问“程序控制方式又分为哪几种”，通常填无条件、查询、中断，不把 DMA 填进去。
...
8086 的 `NMI` 类型号固定为 2，由 CPU 自动产生；`INTR` 的类型号通常由外部中断控制器或外设在中断响应周期提供。

> 易错：`NMI` 虽然来自外部引脚，但它的类型号不是外设给的，也不是 8259A 给的，而是 CPU 固定按类型 2 处理。
...
```text
若 0000:0038H 起连续 4 字节为：
56H, CEH, 12H, F8H

类型号 = 0038H / 4 = 0EH
IP = CE56H
CS = F812H
真正入口地址写作 CS:IP = F812H:CE56H
```

> 易错：有些选择题选项可能故意写成“偏移:段”的顺序，这时要先算出 IP 和 CS，再看选项格式；标准入口地址仍是 `CS:IP`。
...
`INT n` 是软件指令中断，类型号 `n` 写在指令中，由程序主动调用。

> 易错：`INT n` 属于内部/软件中断，不是外设请求产生；它不受 `IF` 标志屏蔽，`IF` 主要控制可屏蔽硬件中断 `INTR`。
...
反过来，若已知 8259A 送出的类型码，也可以用低 3 位判断来自哪个 IR 输入端：
...
若服务程序入口为 `CB08H:4C11H`，写入向量表时按 `IP` 低字节、`IP` 高字节、`CS` 低字节、`CS` 高字节：

```text
IP = 4C11H -> 11H, 4CH
CS = CB08H -> 08H, CBH
向量区 4 个字节：11H, 4CH, 08H, CBH
```

> 易错：ICW2 给的是类型码高 5 位，不是完整指定某一个 IR 的类型号；低 3 位天然对应 IR0~IR7。
