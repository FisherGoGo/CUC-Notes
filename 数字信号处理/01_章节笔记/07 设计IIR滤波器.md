...
本章主线：先设计模拟低通原型滤波器 $H_{a}(s)$，再通过映射方法转化为数字滤波器 $H(z)$。
...
- 对于实滤波器，零极点以共轭复数对形式出现，在 $s$ 平面上以关于虚轴镜像对称存在
---
...
- $\Omega_{p}$ 通带截止频率
- $\Omega_{st}$ 阻带截止频率
- $\delta_{p}$ 通带衰减波纹（dB），$\delta_{p}=20\lg \frac{|H(j0)|}{H(j\Omega_{p})}$
- $\delta_{s}$ 阻带最小衰减（dB），$\delta_{s}=20\lg \frac{|H(j0)|}{H(j\Omega_{st})}$
...
对于截止频率，有
$$
\Omega_{c}=\frac{\Omega_{p}}{\sqrt[2N]{10^{0.1\delta_{p}}-1}}\quad \Omega_{c}=\frac{\Omega_{st}}{\sqrt[2N]{10^{0.1\delta_{s}}-1}}
$$
实际设计时，$\Omega_{c}$ 通常取能同时满足通带和阻带指标的值；若两个公式给出的范围不一致，可在允许范围内取中间值。

2. 根据阶数**查表** 

直接进行查表得到系统函数分母多项式 $s^{N}+a_{N-1}s^{N-1}+\ldots+a_{1}s+a_{0}$ ，其中 $a_{N}=a_{0}=1$ 不变，得到归一化的函数 $H_{a_{N}}(s)$ 
...
1. 给定指标：$\omega_{p}\quad \omega_{st}\quad \delta_{p}\quad \delta_{s}$ 
2. 通过预畸变，确定模拟指标 $\Omega= \dfrac{2}{T}\cdot \tan\left( \frac{\omega}{2} \right)$ ，$T$ 一般取 $2$ 
3. 根据模拟指标设计模拟滤波器 $H_{a}(s)$
...
- 我们有：
$$
e^{-j\theta_{c}}= \frac{e^{-j\omega}-\alpha}{1-\alpha e^{-j\omega}}
$$
- 得到：
$$
...
- 我们可以假设 $\theta_{p}=\pi-\omega_{p}$ ，那么有 $\alpha= -\dfrac{\cos\left(\frac{\theta_{c}+\omega_{c}}{2}\right)}{\cos\left(\frac{\theta_{c}-\omega_{c}}{2}\right)}=0$
