...
N 点圆周卷积也可以理解为线性卷积结果按 $N$ 点周期叠加后取一个主值区间：
...
| 泄漏 | 有限时长截断；记录区间不是周期信号周期的整数倍 | 尽量相干采样，增加记录长度，选合适窗函数 |
| 栅栏效应 | 频域只观察有限个采样点，峰值可能落在采样点之间 | 增加有效记录长度以提高真实分辨率；补零只能加密显示频点 |
...
- $N$：采样点数；
- $T_0=NT$：信号记录长度；
- $F_0=1/T_0=f_s/N$：频率分辨率；
- $f_h$：信号最高频率，并应满足 $f_s\ge 2f_h$。
...
> 易错点：增大有效记录长度 $T_0$ 才能提高真实频率分辨率；只在序列末尾补零会让频谱显示更细，但不会分开原本无法分辨的两个频率分量。
...
8. 频域采样为什么会导致时域周期延拓？
9. 混叠、泄漏、栅栏效应分别来自哪里？
10. $T_0$、$T$、$N$、$f_s$、$F_0$ 之间有什么关系？补零为什么不能提高真实频率分辨率？
